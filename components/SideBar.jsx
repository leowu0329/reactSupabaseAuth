...
function SideBar() {
    ...
    const location = useLocation(); // 用於判斷當前啟用路徑來高亮選單
    ...
    // 輔助函式：判斷是否為當前頁面，並回傳對應的 Bootstrap 樣式
    ...
    return (
        ...
        >
            ...
            <div className={`d-flex align-items-center ${sidebarOpen ? 'justify-content-between p-2' : 'justify-content-center py-2'}`}>
                {sidebarOpen && (
                    <span className="fw-bold text-truncate fs-5 text-uppercase tracking-wider text-info">
                        管理系統
                    </span>
                )}
                <button 
            ...
            <ul className="nav nav-pills flex-column px-1 mb-2">
                <li className="nav-item">
                    <a 
                        ...
                        className={`nav-link text-danger d-flex align-items-center rounded py-3 px-2 ${sidebarOpen ? '' : 'justify-content-center'}`}
